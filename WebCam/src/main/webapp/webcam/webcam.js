...
window.webcam = {
	...
	swf_url : appContextPath+'/webcam/webcam.swf', // URI to webcam.swf movie (defaults to cwd)
	shutter_url : appContextPath+'/webcam/shutter.mp3', // URI to shutter.mp3 sound
	...
	get_html : function(width, height, server_width, server_height) {
		...
		var flashvars = 'shutter_enabled=' + (this.shutter_sound ? 1 : 0)
				+ '&shutter_url=' + escape(this.shutter_url) + '&width='
				+ width + '&height=' + height + '&server_width=' + server_width
	...
	flash_notify : function(type, msg) {
		...
		case 'error':
			...
			if (!this.fire_hook('onError', msg)) {
				...
				$('#cameraSnapBtn').hide();
				$('#cameraErrorMsg').html('Sorry, You dont have Camera.!');
			}
			break;
